...
library(RefManageR)
bib_data <- ReadBib(file = "Final_references.bib")
```
...
options <- BibOptions(bib.style = "authortitle", style = "latex")
print(bib_data, .opts = options)
```
...
\begin{equation}
\underset{j,s}{\min}\left[\underset{c_1}{\min}\sum_{x_i \in R_1(j,s)} (y_i-c_1)^2 + \underset{c_2}{\min} \sum_{x_i \in R_2(j,s)} (y_i-c_2)^2\right]\label{eq:reg}
\end{equation}

For any choice $j$ and $s$, the inner minimization is solved by: 

\begin{equation}
\hat{c}_1 = \text{av}(y_i \,|\, x_i \in R_1(j, s)) \quad \text{and} \quad \hat{c}_2 = \text{av}(y_i \,|\, x_i \in R_2(j, s))\label{eq:crit}
\end{equation}
...
Bootstrap aggregating Regression Trees can be done following the procedure proposed by `r Cite(bib_data, key = "Breiman")`:
