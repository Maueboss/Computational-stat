...
```
There are some scenarios where it may not work or provide significant improvements; for example:
\begin{itemize}
    \item When the dataset is small: we need a large training dataset to create different istances of the base model.
    \item When there is strong dependence among training data: create different istances in this case during bootstrap may not introduce enough diversity. 
...
Watching this easy example of a linear regression model we immediately see that there are some uninformative predictors, so, as we said a few lines ago, we want to verify that bagging doesn't work very well in this case. 
...
We immediately see that the standard error of the bagging is bigger than the standard error of the predictions produced by the original ols model because of the presence of uninformative predictors.
