...
ggpairs(df[,3:10], progress= FALSE, pch="")# We plot only quantitative variables, 
                                          # which in this case are the columns after the third




```
There is not much correlation between the variables, we can have high confidence on this statement (they are all significant)
...
# I create a dataframe (8x8) where each entry is the sample obtained during bootstrap resampling, by creating a list into a list
...
# The reasoning is really similar to the one discussed above, this time we also use the covariance variance matrix of the sample
for(i in 1:6){
  for(j in 1:6){
...
grid.arrange(plot_boot_sample_ci(1,1),plot_boot_sample_ci(2,3))
```
...
summary(lm_game)
```
The estimated intercept is really low, 0.008 approximately, also it should be considered 0 because the p-value under the null hypothesis stated as $H_0:\beta_0=0$ against $H_1:\beta_0=0$ is around 0.5 (we reject the null hypothesis )

```{r}
# store the values obtained through the sample that will be computed afterwards using bootstrap
...
\begin{figure}[h!]
    \centering
    \includegraphics[scale =0.5]{Screenshot 2023-06-01 163530.png}
    \caption{Type I error vs Type II error}
    \label{fig: n=2}
\end{figure}
