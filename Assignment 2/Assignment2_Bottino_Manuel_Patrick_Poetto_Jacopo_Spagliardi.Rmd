...
autori <- unique(df2$first_auth)

# Creazione della matrice delle comparazioni
matrice_comparazioni <- matrix(NA, nrow = length(autori), ncol = length(autori), dimnames = list(autori, autori))

# Loop per calcolare le comparazioni
for (i in 1:length(autori)) {
  for (j in 1:length(autori)) {
    if (i != j) {
      diff <- pop_est(df2, autori[i], autori[j])
      matrice_comparazioni[i, j] <- diff
    }
  }
}
```
...
```{r}
heatmap <- ggplot(data = melt(matrice_comparazioni)) +
